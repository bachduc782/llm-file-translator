...
_OVERLAP_CHARS = 200  # 前チャンクからコンテキストとして渡す文字数
...
def _estimate_max_tokens(items: list[str]) -> int:
    """入力文字数から必要な max_tokens を推定する。"""
    total_chars = sum(len(s) for s in items)
    # 翻訳後の文字数 ≒ 入力の1.2倍、3文字/トークン換算 + 512バッファ
...
def _translate_unique(
    ...
    short_texts = [t for t in unique if len(t) <= CELL_SPLIT_THRESHOLD]
    if long_texts:
        progress(f"  長いテキスト {len(long_texts)}件を段落分割して翻訳")
    ...
    def _worker(chunk: list, idx: int):
        ...
        with lock:
            ...
            progress(f"[{idx}/{total_chunks}] 完了 — {done}/{len(short_texts)}件翻訳済み")
    ...
    for text in long_texts:
        progress(f"  長文分割翻訳中 ({len(text)}文字)…")
